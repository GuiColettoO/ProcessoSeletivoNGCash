...
import {api, createSession, LoginSession} from "../services/api";
...
export const AuthProvider = ({children}) => {
    ...
    const signup = async (username, password) => {
        const usersStorage = await createSession(username, password)
    
        const hasUser = usersStorage?.filter((user) => user.username === username);
    
        if (hasUser?.length) {
          return "Já tem uma conta com esse username";
        }
